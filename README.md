...
全端網頁應用，協助連鎖品牌（便利商店、超市藥妝）以數據科學方法評估展店位點潛力。

---

## 一、發想

零售業拓點常面臨「直覺選址」導致的高失敗風險。本專案從數據分析師視角出發，將選址問題拆解為**人口、消費、競爭、據點、成本**五大維度。

由於外部人員難以取得企業內部營收，我們轉而挖掘 Open Data 的潛力，以**財政部電子發票指標**替代消費熱度，旨在建立一套可量化的科學決策基準，協助企業評估新店點的生存潛力。

## 二、使用的技術

分析流程以 Python 為核心，運用 **XGBoost** 處理「營運」與「廢止」高度重疊的非線性邊界問題。針對空間數據，整合 **TGOS 與 Google Maps API** 進行座標轉化與距離運算。開發端則實踐 **Vibe Coding** 理念，利用 LLM 輔助生成 Angular 前端儀表板與 FastAPI 服務，並整合 **Gemini API** 將模型輸出自動轉化為商業洞察建議。

## 三、思考流程

**深耕真實數據的處理經驗**：因實際限制，使用 Open Data 為替代指標，考慮到雜訊與偏態問題使用相對比例降低干擾。此外，因「倖存者偏差」問題，本模型僅能學習「已開點位」的成功與失敗，應進一步強化特徵工程以捕捉更微觀的商圈環境訊號。

**數據驅動的雙軌商業決策**：基於不同零售業態對風險的容忍度差異，設計「雙軌推薦門檻」：便利商店重視市佔擴張（門檻 0.5），超市藥妝則側重高質量選址（門檻 0.7）。並透過 **SHAP Value** 拆解模型黑箱，將特徵貢獻度轉化為直觀的「雷達圖」，讓非技術決策者也能一眼掌握商圈體質的強弱分佈。

**AI 賦能的敏捷分析實踐（Vibe Coding）**：作為轉職者，我專注於「商業邏輯與特徵定義」，而將繁瑣的系統實作交給 AI。透過「描述分析流程 → AI 生成程式架構 → 人工微調驗證」的 Vibe Coding 技巧，在四週內完成從資料 ETL 到 GCP Cloud Run 佈建的全流程，證明了 AI 時代下分析師能具備極高的端到端（End-to-End）產出效率。
...
- **Loader 載入時間**：1.10s → 0.04s（加速 27 倍）
- **整體冷啟動時間**：7.17s → ~6.2s（減少 ~1 秒，包括 Cloud Run 容器啟動時間）
